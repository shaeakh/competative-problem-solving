...
void solve(){  
    ...
    while (t--)
    {
        ...
        for (int i = 0; i < n-1; i++)
        {
            ...
            ll timeTojump = arr[i+1] - arr[i]; // arr[i] = 1
            ll water_level_catch = arr[i] - water_level+1;
            // printf("timeTojump: %lld, water_level_catch: %lld\n", timeTojump, water_level_catch);

            if(timeTojump > water_level_catch) {
                flag = false;
                break;
            }
        }
